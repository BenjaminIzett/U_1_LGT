...
# function inv_FK_3d_old(Lx, Ly, Lz, M)
#     return reshape([1 / (4 * sin(π * k1 / Lx)^2 + 4 * sin(π * k2 / Ly)^2 + 4 * sin(π * k3 / Lz)^2 + M^2) for k3 in 0:Lz-1 for k2 in 0:Ly-1 for k1 in 0:Lx-1], (Lx, Ly, Lz))
# end

function fa_sample_momenta_3d(inverse_FK)
    ...
    # Π = randn(inverse_FK_size) .* sqrt.((Lx * Ly * Lz) ./ inverse_FK)

    # p = zeros(inverse_FK_size)
    # p[1, :, :, :] = real(FFTW.ifft(fourier_space_momenta_3d(Π_1, Lx, Ly, Lz)))
    # p[2, :, :, :] = real(FFTW.ifft(fourier_space_momenta_3d(Π_2, Lx, Ly, Lz)))
    # p[3, :, :, :] = real(FFTW.ifft(fourier_space_momenta_3d(Π_3, Lx, Ly, Lz)))
...
function fa_hmc_run_3d(ϕ, S, dSdϕ, inverse_FK, lf_steps, Δτ, S_args)
    ...
    Nsites = Lx * Ly * Lz
    fft_dims = (2, 3, 4)
    Fp_0 = fa_sample_momenta_3d(inverse_FK)
...
# function fourier_space_momenta_3d_old(inverse_FK, Lx, Ly, Lz)
#     # Not optimal
#     # Assumes Lx,Ly,Lz are all even

#     # Can do to remove Lx,Ly,Lz from arguments
#     # (Lx, Ly, Lz) = size(Π)
#     Π = map(Ak -> randn() * sqrt((Lx * Ly * Lz) / Ak), inverse_FK)
...
# mean([sum((fafa_ke(inverse_FK,fft(HMC.fa_sample_momenta_3d(inverse_FK),(2,3,4)))))/16^6 for _ in 1:1000])


# mean([((fafa_ke(inverse_FK,fft(HMC.fa_sample_momenta_3d(inverse_FK),(2,3,4)))))/16^3 for _ in 1:1000])
